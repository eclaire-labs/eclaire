...
import type { StreamEvent, DiscordLogger } from "./deps.js";
...
/**
 * Streams AI response text into a Discord channel by sending an initial message
...
export async function sendStreamingResponse(
  ...
  async function sendInitialMessage(): Promise<Message> {
    const msg = await withRetry(() => channel.send(CURSOR));
    sentMessages.push(msg);
    return msg;
  }

  async function editMessage(text: string, msg: Message): Promise<void> {
    if (text === lastEditedText) return;

    try {
      await withRetry(() => msg.edit(text));
      lastEditedText = text;
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : "";
      logger.warn(
        { channelId: channel.id, messageId: msg.id, error: errMsg },
        "Failed to edit streaming message",
      );
    }
  }

  async function startNewMessage(): Promise<Message> {
