...
export const DiscordConfigSchema = z
  .object({
    ...
    voice_mode: z.enum(["listen", "speak", "both"]).default("both").meta({
      description:
        'Voice channel behavior: "listen" to transcribe users, "speak" to play responses, "both" for full two-way',
    }),
    stt_enabled: z.boolean().default(true).meta({
      description:
        "Whether to transcribe voice channel audio via speech-to-text",
    }),
  })
  .strict()
  .meta({
...
export function validateAndEncryptConfig(
  ...
  return {
    ...
    ...(validated.voice_channel_id && {
      voice_channel_id: validated.voice_channel_id,
    }),
 ...
 */
export function decryptConfig(storedConfig: unknown): DiscordConfig | null {
  ...
  try {
    ...
    const { channel_id, bot_token } = storedConfig as Record<string, unknown>;
