...
describe.each(DB_TEST_CONFIGS)(
  "A4, A5, A6, A7: Timing and Lease ($label)",
  ({ dbType }) => {
    ...
    describe("A4: Priority Ordering", () => {
      it("should process higher priority jobs first", async () => {
        ...
        const job1 = await client.enqueue("test-queue", { priority: 1 }, { priority: 1 });
        const job2 = await client.enqueue("test-queue", { priority: 5 }, { priority: 5 });
        const job3 = await client.enqueue("test-queue", { priority: 10 }, { priority: 10 });
      ...
      it("should process same-priority jobs in FIFO order", async () => {
        ...
        const job1 = await client.enqueue("test-queue", { order: 1 }, { priority: 5 });
        await sleep(10); // Ensure different createdAt
        const job2 = await client.enqueue("test-queue", { order: 2 }, { priority: 5 });
        await sleep(10);
        const job3 = await client.enqueue("test-queue", { order: 3 }, { priority: 5 });
