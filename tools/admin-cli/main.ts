...
import { setLoggerFactory, setConfigPath, type AILogger } from '@eclaire/ai';
...
// CLI Header
console.log(boxen(
  chalk.cyan.bold('Eclaire Admin CLI'),
  {
    padding: 1,
    margin: 1,
    borderStyle: 'round',
...
program
  .name('eclaire')
  .description('Eclaire Admin CLI - Manage providers, models, and configuration')
  .version('1.0.0')
...
program.hook('preAction', (thisCommand) => {
  ...
  const loggerFactory = options.verbose ? createVerboseLogger : () => silentLogger;
...
program.exitOverride((err) => {
  if (err.code === 'commander.unknownCommand') {
    ...
    console.log(chalk.gray('  Run') + chalk.cyan(' eclaire --help ') + chalk.gray('to see available commands\n'));
    process.exit(1);
  }
