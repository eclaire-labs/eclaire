...
export async function upCommand(options: UpOptions = {}): Promise<void> {
  try {
    ...
    if (resolution.status === 'no-managed') {
      ...
      console.log(colors.dim('  Configure a managed llama-cpp provider in providers.json'));
      console.log(colors.dim('  and select a model using that provider in selection.json'));
      return;
    }

    if (resolution.status === 'conflict') {
      console.log(colors.error(`${icons.error} ${resolution.message}`));
      console.log(colors.dim('  Update selection.json so all models use the same managed provider'));
      process.exit(1);
    }
    ...
    if (status.running) {
      console.log(colors.info(`${icons.info} llama-cpp engine is already running (PID: ${status.pid})`));
      return;
    }

    // Run pre-flight memory check
    const memoryOk = await runMemoryPreflight(resolution, options.force ?? false);
    if (!memoryOk) {
      process.exit(1);
    }
    ...
    const modelList = resolution.modelsToPreload.map(m =>
      `  - ${colors.info(m.context)}: ${m.providerModel}`
    ).join('\n');
    ...
    try {
      ...
      spinner.succeed(`llama-cpp engine started (PID: ${pid}, port: ${settings.port})`);
    } catch (error: any) {
    ...
    console.log(colors.error(`${icons.error} Failed to start engine: ${error.message}`));
    process.exit(1);
  }
}
...
function displayMemoryCheck(result: MemoryCheckResult): void {
  ...
  console.log(`  GPU: ${colors.emphasis(gpuName)} (${formatMemorySize(result.vramStatus.totalVRAM)} ${memoryType})`);
  console.log(`  Available: ~${formatMemorySize(result.vramStatus.availableVRAM)} (Metal GPU budget)\n`);
  ...
  if (result.details.backend) {
    ...
    if (est.confidence !== 'low') {
      ...
      console.log(`    Model weights:    ${formatMemorySize(est.modelWeights)}`);
      console.log(`    KV cache:         ${formatMemorySize(est.kvCache)}`);
      console.log(`    Compute buffers:  ${formatMemorySize(est.computeBuffers)}`);
      console.log(colors.dim('    ───────────────────────'));
      console.log(`    Subtotal:         ${formatMemorySize(est.total)}\n`);
    } else {
      console.log(`  ${colors.info('Backend')}: ${colors.dim('Unknown (model size not available)')}\n`);
    }
  }
  ...
  if (result.details.workers) {
    ...
    if (est.confidence !== 'low') {
      ...
      console.log(`  ${colors.success('Workers')}: ${colors.dim('Unknown (model size not available)')}\n`);
    }
  }
  ...
  if (result.requiredVRAM > 0) {
    console.log(colors.dim('  ═══════════════════════════════'));
    console.log(`  Total Required:     ${formatMemorySize(result.requiredVRAM)}`);
    console.log(`  Available:          ${formatMemorySize(result.availableVRAM)}`);

    if (result.headroom >= 0) {
      console.log(`  Headroom:           ${formatMemorySize(result.headroom)}`);
    } else {
      console.log(`  ${colors.error('Deficit:')}            ${formatMemorySize(Math.abs(result.headroom))}`);
    }
  }
  ...
  if (result.status === 'ok') {
    ...
    console.log(colors.warning(`  ${icons.warning} WARNING: ${result.message}`));
    console.log('');
