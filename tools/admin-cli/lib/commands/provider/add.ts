...
import { PROVIDER_PRESETS, getPresetById, getPresetsForSelection } from '../../config/presets.js';
import {
...
import type { CommandOptions, ProviderConfig, ProviderPreset, EngineConfig } from '../../types/index.js';
...
export async function addCommand(options: CommandOptions): Promise<void> {
  try {
    ...
    if (options.preset) {
      ...
      if (!foundPreset) {
        console.log(colors.error(`${icons.error} Unknown preset: ${options.preset}`));
        console.log(colors.dim('Available presets: ' + PROVIDER_PRESETS.map(p => p.id).join(', ')));
        process.exit(1);
      }
      preset = foundPreset;
    ...
    if (preset.id === 'custom') {
      ...
      config.baseUrl = await promptBaseUrl('Base URL (include /v1 for OpenAI-compatible):', preset.config.baseUrl);
      config.dialect = await promptDialect('API Dialect:', preset.config.dialect);
      config.auth.type = await promptAuthType('Authentication Type:', 'none');

      if (config.auth.type === 'bearer' || config.auth.type === 'header') {
        const apiKey = await promptApiKey();
        config.auth.header = config.auth.type === 'bearer' ? 'Authorization' : await promptHeaderName();
        config.auth.value = config.auth.type === 'bearer' ? `Bearer ${apiKey}` : apiKey;
      }
      ...
      if (!preset.isCloud && preset.defaultPort) {
        ...
        if (!useDefaultPort.useDefault) {
          const port = await promptPort('Enter port number:', preset.defaultPort);
      ...
      if (preset.config.auth.requiresApiKey) {
        ...
        if (preset.config.auth.envVar) {
          ...
          config.auth.header = preset.config.auth.type === 'bearer' ? 'Authorization' : preset.config.auth.envVar === 'ANTHROPIC_API_KEY' ? 'x-api-key' : 'Authorization';
          config.auth.value = preset.config.auth.type === 'bearer' ? `Bearer ${apiKey}` : apiKey;
        } else {
      ...
      if (preset.defaultEngine) {
        ...
        if (canBeManaged) {
          ...
          config.engine = {
            ...
            ...(managed && preset.defaultEngine.gpuLayers !== undefined && { gpuLayers: preset.defaultEngine.gpuLayers }),
            ...(managed && preset.defaultEngine.contextSize !== undefined && { contextSize: preset.defaultEngine.contextSize }),
            ...(managed && preset.defaultEngine.batchSize !== undefined && { batchSize: preset.defaultEngine.batchSize }),
          };
        } else {
          ...
          console.log(colors.dim(`Engine '${preset.defaultEngine.name}' is external - Eclaire will connect but not start/stop it`));
        }
      }
    }
    ...
    if (!preset.isCloud) {
      ...
      console.log(colors.dim(`Test connectivity: eclaire provider test ${providerId}`));
    } else {
    ...
    console.log(colors.error(`${icons.error} Failed to add provider: ${error.message}`));
    process.exit(1);
  }
}
