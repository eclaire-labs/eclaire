...
import type { CommandOptions, ProviderConfig, Dialect } from '../../types/index.js';
...
export async function testCommand(id: string, options: CommandOptions): Promise<void> {
  try {
    ...
    if (result.success) {
      ...
      switch (result.errorType) {
        ...
        case 'auth':
          ...
          console.log(colors.dim('  - API key not authorized for this endpoint'));
          break;
    ...
    console.log(colors.error(`${icons.error} Failed to test provider: ${error.message}`));
    process.exit(1);
  }
}
...
async function testProviderConnectivity(
  ...
  const endpoint = provider.overrides?.chatPath ?? getEndpointForDialect(provider.dialect);
  ...
  if (provider.auth.type !== 'none' && provider.auth.header && provider.auth.value) {
  ...
  try {
    ...
    return {
      ...
      status: response.status,
      latencyMs,
      error: response.data?.error?.message || response.data?.detail || 'Server error',
      errorType: 'server'
    };
