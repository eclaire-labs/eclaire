...
import { getActiveModels, getActiveModelsAsObjects, getModels, setActiveModel, removeActiveModel, findModelById, isModelSuitableForBackend, isModelSuitableForWorkers, isModelSuitableForContext } from '../../config/models.js';
  ...
  try {
    ...
    if (options.backend || options.workers) {
      if (options.backend) {
        ...
        if (!model) {
          console.log(colors.error(`${icons.error} Model not found: ${options.backend}`));
          process.exit(1);
        }
        if (!isModelSuitableForBackend(model)) {
          console.log(colors.error(`${icons.error} Model ${options.backend} is not suitable for backend context (requires text input)`));
          process.exit(1);
        }
        setActiveModel('backend', options.backend);
        console.log(colors.success(`${icons.success} Backend active model set to ${options.backend}`));
      ...
      if (options.workers) {
        ...
        if (!model) {
          console.log(colors.error(`${icons.error} Model not found: ${options.workers}`));
          process.exit(1);
        }
        if (!isModelSuitableForWorkers(model)) {
          console.log(colors.error(`${icons.error} Model ${options.workers} is not suitable for workers context (requires text + image input)`));
          process.exit(1);
        }
        setActiveModel('workers', options.workers);
        console.log(colors.success(`${icons.success} Workers active model set to ${options.workers}`));
    ...
    if (id) {
      ...
      console.log(colors.info(`Supported Contexts: ${supportedContexts.join(', ')}`));
      ...
      if (supportedContexts.length === 1) {
        ...
        console.log(colors.success(`${icons.success} ${context} active model set to ${id}`));
...
async function setActiveForContext(
  ...
  const availableModels = allModels.filter(({ model }) => isModelSuitableForContext(model, context));
