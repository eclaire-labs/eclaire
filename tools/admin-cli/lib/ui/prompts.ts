...
import { colors, formatProvider, truncateString, formatEngine } from './colors.js';
import type { Model, Context, ProviderPreset, ProviderConfig, Dialect } from '../types/index.js';
  ...
  const { dialect } = await inquirer.prompt([
    {
      ...
      message,
      choices: [
        { name: 'OpenAI-compatible (OpenAI, OpenRouter, llama.cpp, etc.)', value: 'openai_compatible' },
        { name: 'Anthropic Messages API (Claude)', value: 'anthropic_messages' },
...
export async function promptProviderFields(
  ...
  for (const field of fieldsToEdit) {
    switch (field) {
      case 'baseUrl':
        updates.baseUrl = await promptBaseUrl('New base URL:', currentConfig.baseUrl);
        break;
      case 'dialect':
        updates.dialect = await promptDialect('New dialect:', currentConfig.dialect);
        break;
      case 'auth':
        const authType = await promptAuthType('New auth type:', currentConfig.auth.type);
        ...
        if (authType === 'bearer' || authType === 'header') {
          ...
          const headerName = authType === 'bearer' ? 'Authorization' : await promptHeaderName('Header name:', currentConfig.auth.header);
