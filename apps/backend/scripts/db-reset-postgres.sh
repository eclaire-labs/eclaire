...
# Works in both local development (via docker exec) and container environments
...
echo "🔄 Resetting PostgreSQL database with '$SEED_TYPE' data..."
echo ""

# Detect if we're running inside a container or from the host
# Check if we're inside a container by looking for /.dockerenv or checking docker.sock availability
if [ -f /.dockerenv ] || [ "$RUNNING_IN_CONTAINER" = "true" ]; then
    CONTAINER_MODE=true
    echo "📦 Running in container mode"
else
    CONTAINER_MODE=false
    echo "🖥️  Running in local mode (host machine)"
fi
...
if [ "$CONTAINER_MODE" = true ]; then
    # ============================================================================
    # CONTAINER MODE: Running inside a container (production/staging)
    # ============================================================================
    ...
    # ============================================================================
    # LOCAL MODE: Running from host machine with Docker container
    # ============================================================================
