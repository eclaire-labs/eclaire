...
export interface EclaireConfig {
  ...
  security: {
    ...
    secretsAutoGenerated: boolean; // True if secrets were auto-generated for dev
  };
...
/**
 * Generate an API key in the expected sk-{keyId}-{secret} format
 * - keyId: 15 alphanumeric characters (uppercase)
 * - secret: 32 alphanumeric characters
 */
function generateApiKeyFormat(): string {
  const keyIdArray = new Uint8Array(8);
  crypto.getRandomValues(keyIdArray);
  const keyId = Array.from(keyIdArray, (b) => b.toString(16).padStart(2, "0"))
    .join("")
    .substring(0, 15)
    .toUpperCase();
  ...
  const secret = Array.from(secretArray, (b) =>
    b.toString(16).padStart(2, "0"),
  ).join("");

  return `sk-${keyId}-${secret}`;
}
...
const TEST_SECRETS = {
  ...
  // API keys must follow sk-{15chars}-{32chars} format for auth validation
  workerApiKey: "sk-DEVONLYWORKER01-DEVONLY0000000000000000000000000",
  aiAssistantApiKey: "sk-DEVONLYAIASST01-DEVONLY1111111111111111111111111",
};
...
function persistSecretsToEnvLocal(
  ...
  const lines = [
    ...
    `WORKER_API_KEY=${secrets.workerApiKey}`,
    `AI_ASSISTANT_API_KEY=${secrets.aiAssistantApiKey}`,
    "",
  ];
...
export function buildConfig(): EclaireConfig {
  ...
  const secrets = {
    ...
    aiAssistantApiKey: env.AI_ASSISTANT_API_KEY || "",
  };
  ...
  if (!isProduction) {
    if (isTest) {
      ...
      if (!secrets.workerApiKey)
        secrets.workerApiKey = TEST_SECRETS.workerApiKey;
      if (!secrets.aiAssistantApiKey)
        secrets.aiAssistantApiKey = TEST_SECRETS.aiAssistantApiKey;
    } else {
      ...
      if (!secrets.workerApiKey) {
        secrets.workerApiKey = generateApiKeyFormat();
        secretsAutoGenerated = true;
      }
      if (!secrets.aiAssistantApiKey) {
        secrets.aiAssistantApiKey = generateApiKeyFormat();
        secretsAutoGenerated = true;
      }
  ...
  const config: EclaireConfig = {
    ...
    worker: {
      ...
      apiKey: secrets.workerApiKey,
      aiAssistantApiKey: secrets.aiAssistantApiKey,
      sharedDataPath:
...
export function validateConfig(config: EclaireConfig): string[] {
  ...
  // Worker keys required when running workers
  if (config.serviceRole === "worker" || config.serviceRole === "all") {
    requiredSecrets.push(
      { key: "WORKER_API_KEY", value: config.worker.apiKey, minLen: 32 },
      { key: "AI_ASSISTANT_API_KEY", value: config.worker.aiAssistantApiKey, minLen: 32 },
    );
  }
  ...
  if (config.isProduction) {
    ...
    // Development: permissive, just note about auto-generated secrets
    if (config.security.secretsAutoGenerated) {
      warnings.push(
        "Auto-generated secrets saved to .env.local (gitignored, will persist across restarts)",
      );
    }
  }
