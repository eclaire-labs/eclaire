...
export async function createTask(taskData: CreateTaskParams, userId: string) {
  try {
    ...
    const newTask = await db.transaction(async (tx) => {
      ...
      if (!taskEntry) {
        throw new Error("Failed to create task entry");
      }

      // 4. Create the processing job record in the same transaction
      await tx.insert(assetProcessingJobs).values({
        assetType: "tasks",
        assetId: taskEntry.id,
        userId: userId,
        status: "pending",
    ...
    if (!newTask) {
      throw new Error("Failed to create task in transaction");
    }

    const taskId = newTask.id; // Use the generated ID
    ...
    await recordHistory({
      ...
      // Use the actual inserted data (including generated timestamps) for 'afterData'
      // but format it for consistency if needed, or just pass raw data
      afterData: { ...newTask, dueDate: taskData.dueDate, tags: taskData.tags }, // Pass original input dueDate for history? Or formatted? Decide consistency.
      actor: "user",
      userId: userId,
    });
...
export async function updateTaskArtifacts(
  ...
  try {
    ...
    // Since addTagsToTask handles the logic of clearing and adding, we can use it.
    // However, it's often cleaner to have the logic self-contained here.
    await db.transaction(async (tx) => {
      ...
      await tx.delete(tasksTags).where(eq(tasksTags.taskId, taskId));

      // Add new tags using the helper if they exist - PASS THE TX OBJECT!
      if (artifacts.tags && artifacts.tags.length > 0) {
        await addTagsToTask(taskId, artifacts.tags, task.userId, tx);
      }
