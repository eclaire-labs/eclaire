...
export async function createTask(taskData: CreateTaskParams, userId: string) {
  try {
    ...
    // Execute transaction with synchronous callback
...
export async function updateTaskArtifacts(
  ...
  try {
    ...
    // Now execute the synchronous transaction
    await txManager.withTransaction((tx) => {
      // Clear existing tags for a full replacement
      tx.tasksTags.delete(eq(tasksTags.taskId, taskId));

      // Insert new tags
      if (tagList.length > 0) {
        tagList.forEach((tag) => {
          tx.tasksTags.insert({ taskId, tagId: tag.id });
        });
      }

      // Update the task's timestamp
      tx.tasks.update(eq(tasks.id, taskId), { updatedAt: new Date() });
    });
  } catch (err) {
    logger.error(
      {
