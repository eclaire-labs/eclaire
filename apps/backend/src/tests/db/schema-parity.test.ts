	...
	({ dbType, label }) => {
		...
		describe("Foreign Key Constraints", () => {
			...
			it("should cascade delete on parent deletion", async () => {
				...
				// Delete bookmark
				if (dbType === "sqlite") {
					await db.delete(sqliteSchema.bookmarks).where(
						eq(sqliteSchema.bookmarks.id, bookmarkId),
					);
				} else {
					await db.delete(pgSchema.bookmarks).where(
						eq(pgSchema.bookmarks.id, bookmarkId),
					);
				}
