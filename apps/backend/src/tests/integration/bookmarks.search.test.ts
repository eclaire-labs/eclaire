...
describe("Bookmark Search & Filter Integration Tests", {
  timeout: 30000,
}, () => {
  ...
  it("should find bookmarks by title text search", async () => {
    ...
    const found = data.items.find((b) => b.title.includes("Alpha"));
  ...
  it("should find bookmarks by description text search", async () => {
    ...
    const found = data.items.find((b) => b.title.includes("Beta"));
    expect(found).toBeDefined();
  });
  ...
  it("should find bookmarks by URL text search", async () => {
    ...
    const found = data.items.find((b) => b.title.includes("Gamma"));
    expect(found).toBeDefined();
  });
  ...
  it("should filter bookmarks by single tag", async () => {
    ...
    const found = data.items.find((b) => b.title.includes("Rust"));
    expect(found).toBeDefined();
  ...
  it("should respect limit parameter", async () => {
    const response = await authenticatedFetch(`${BASE_URL}/bookmarks?limit=1`, {
      method: "GET",
    });
