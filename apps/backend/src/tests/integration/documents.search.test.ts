...
describe("Document Search & Filter Integration Tests", {
  timeout: 30000,
}, () => {
  ...
  it("should find documents by due date range", async () => {
    ...
    const found = data.items.find(
      (d) =>
        d.dueDate &&
        new Date(d.dueDate) <= new Date(Date.now() + 10 * 24 * 60 * 60 * 1000),
    );
    expect(found).toBeDefined();
  });
  ...
  it("should respect limit parameter", async () => {
    const response = await authenticatedFetch(`${BASE_URL}/documents?limit=1`, {
      method: "GET",
    });
