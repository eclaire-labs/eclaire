...
const { handleIncomingMessage, handleIncomingVoiceMessage } =
  await import("../../../../../../packages/channels/telegram/src/incoming.js");
const { setDeps } =
  await import("../../../../../../packages/channels/telegram/src/deps.js");
const { stopBot } =
  await import("../../../../../../packages/channels/telegram/src/bot-manager.js");
    ...
    describe("handleIncomingVoiceMessage", () => {
      it("processes voice message through full pipeline", async () => {
        ...
        try {
          ...
          expect(ctx.telegram.getFileLink).toHaveBeenCalledWith(
            "voice-file-123",
          );
          ...
          const historyRows = await testDb.db.query.history.findMany();
          expect(historyRows.length).toBe(1);
          expect(historyRows[0].action).toBe(
            "telegram_voice_message_processed",
          );
        } finally {
